...
public class FlywheelSysIdCommands {
  ...
  public static Command quasistatic(Flywheel flywheel, SysIdRoutine.Direction direction) {
    ...
    return warmup(flywheel)
        .andThen(
            withResults(
                createRoutine(flywheel, appliedVolts).quasistatic(direction),
                flywheel,
                testName,
                appliedVolts));
  }
  ...
  public static Command dynamic(Flywheel flywheel, SysIdRoutine.Direction direction) {
    ...
    return warmup(flywheel)
        .andThen(
            withResults(
                createRoutine(flywheel, appliedVolts).dynamic(direction),
                flywheel,
                testName,
                appliedVolts));
  }
  ...
  private static Command withResults(
      Command sysIdCommand, Flywheel flywheel, String testName, double[] appliedVolts) {
    SysIdRunStats stats =
        new SysIdRunStats(flywheel::getPosition, flywheel::getVelocity, appliedVolts);
    return sysIdCommand
