...
public class PositionJointSysIdCommands {
  ...
  public static Command quasistatic(PositionJoint positionJoint, SysIdRoutine.Direction direction) {
    ...
    return warmup(positionJoint)
        .andThen(
            withResults(
                createRoutine(positionJoint, appliedVolts).quasistatic(direction),
                positionJoint,
                testName,
                appliedVolts));
  }
  ...
  public static Command dynamic(PositionJoint positionJoint, SysIdRoutine.Direction direction) {
    ...
    return warmup(positionJoint)
        .andThen(
            withResults(
                createRoutine(positionJoint, appliedVolts).dynamic(direction),
                positionJoint,
                testName,
                appliedVolts));
  }

  private static SysIdRoutine createRoutine(
      PositionJoint positionJoint, double[] appliedVolts) {
  ...
  private static Command withResults(
      Command sysIdCommand,
      PositionJoint positionJoint,
      String testName,
      double[] appliedVolts) {
    SysIdRunStats stats =
        new SysIdRunStats(positionJoint::getPosition, positionJoint::getVelocity, appliedVolts);
    return sysIdCommand
        .deadlineFor(Commands.run(stats::sample))
