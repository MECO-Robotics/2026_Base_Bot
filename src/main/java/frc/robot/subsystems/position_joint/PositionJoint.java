 ...
 *
 * <p>This class owns position goals and tunable gains, while the backing {@link PositionJointIO}
...
public class PositionJoint extends SubsystemBase {
  ...
  public void periodic() {
    ...
    if (voltageMode) {
      // Open-loop commands own the output until a new position goal is requested.
    } else if (complianceAfterTarget && (atTarget || complianceActive)) {
      if (!complianceActive) {
  ...
  /** Clears any temporary profile constraint override and restores tunable defaults. */
  ...
  /**
   * Builds a command that sets position with a temporary max-velocity limit and optional compliance
   * mode after reaching target.
   */
  public static Command setPosition(
      PositionJoint positionJoint,
      DoubleSupplier positionSupplier,
    ...
    return new PositionJointPositionCommand(
        positionJoint, positionSupplier, maxVelocitySupplier, complianceAfterTarget);
  }

  /** Builds a command that optionally enters compliance once target tolerance is reached. */
  public static Command setPosition(
      PositionJoint positionJoint, DoubleSupplier positionSupplier, boolean complianceAfterTarget) {
    return new PositionJointPositionCommand(
        positionJoint, positionSupplier, null, complianceAfterTarget);
  }
